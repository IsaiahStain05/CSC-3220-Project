  ...
  observeEvent(input$predict, {
    ...
    output$prediction_result <- renderUI({
      if (predicted_class$.pred_class == "TRUE") {
        div(class = "move-up", "💰 Predicted income class: Makes more than $50K per Year")
      } else {
        div(class = "move-down", "Predicted income class: Makes less than or equal to $50K per Year")
      }
    })
  })
}